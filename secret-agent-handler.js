...
function getBanner() {
  return new AttachmentBuilder('./secret-agent-banner.png', { name: 'secret-agent-banner.png' });
}
...
function buildLobbyEmbed(game, guild) {
  ...
  return new EmbedBuilder()
    ...
    .setImage('attachment://secret-agent-banner.png')
...
function buildVotingEmbed(game, timeLeft) {
  ...
  return new EmbedBuilder()
    ...
    .setFooter({ text: 'Ciyaal Xamar Bot · Secret Agent · Codeynta hoose ka samee!' });
}
...
function buildChannelVoteButtons(game) {
  const players = Array.from(game.players.values());
  const rows = [];
  for (let i = 0; i < players.length && rows.length < 5; i += 4) {
...
async function startVoting(client, game, channel) {
  ...
  // DM-ga: vote buttons u dir ciyaaryahan walba
  for (const player of game.players.values()) {
    const user = await client.users.fetch(player.id).catch(() => null);
    if (!user) continue;
    await user.send({
  ...
  // Channel-ka: vote buttons u dir si channel-ka lagaga codeyn karo
  await channel.send({
    embeds: [
      new EmbedBuilder()
        .setTitle('🗳️ CODEYNTA WAA BILAABATAY!')
        .setColor(0xc9184a)
        .setDescription(
          '**Yaa Secret Agent-ka?** Dooro ciyaaryahanka aad u malaynayso!\n\n' +
          '📩 DM-kaaga ayaad sidoo kale kaga codeyn kartaa.\n' +
          '🔄 Waxaad **bedeli kartaa** codkaaga 90 ilbiriqsi gudahood.'
        )
        .setFooter({ text: 'Ciyaal Xamar Bot · Secret Agent' })
    ],
    components: buildChannelVoteButtons(game),
  }).catch(() => null);

  // Live voting tally embed channel-ka
...
async function refreshLobby(game, guild, channel) {
  ...
  if (msg) await msg.edit({
    embeds: [buildLobbyEmbed(game, guild)],
...
export async function handleSecretAgentMessage(client, msg) {
  ...
  // ── !jooji — Ciyaarta joogsi (lobby iyo ciyaarta intay socoto labadaba) ────
  if (content === '!jooji') {
    const channelId = msg.channel.id;
    const game = agentGames.get(channelId);

    if (!game || game.phase === 'ended') {
      await msg.reply('⚠️ Kanaalkan ciyaaro Secret Agent ah ma socoto.');
      return true;
    }

    if (msg.author.id !== game.hostId) {
      await msg.reply('⚠️ Kaliya **host-ku** wuxuu joojin karaa ciyaarta.');
      return true;
    }
    ...
    // Lobby message-ka buttons ka saar
    if (game.lobbyMessageId) {
      const lm = await msg.channel.messages.fetch(game.lobbyMessageId).catch(() => null);
      if (lm) await lm.edit({ components: [] }).catch(() => null);
    }

    await msg.channel.send({ embeds: [
      new EmbedBuilder()
        .setTitle('🛑 CIYAARTA WAA LA JOOJIYAY')
        .setColor(0xed4245)
        .setDescription(
          `Host-ku wuxuu joojiyay ciyaarta Secret Agent.\n\n` +
          `📍 Phase-kii: **${phase}**\n` +
          `Ciyaara cusub bilaab: \`!qarsoon\``
        )
        .setFooter({ text: 'Ciyaal Xamar Bot · Secret Agent' })
        .setTimestamp()
    ]}).catch(() => null);
