...
client.on(Events.GuildMemberAdd, async (member) => {
  pushLog('join', `${member.user.username} wuxuu ku biiray`, member.guild.name, member.user.id, member.user.username);
  const channel = member.guild.systemChannel;
  if (!channel) return;
  const embed = new EmbedBuilder()
    .setTitle('👋 Soo Dhawoow — Ciyaal Xamar!')
    .setDescription(
      `Soo dhawoow **${member.displayName}** server-ka **${member.guild.name}**!\n\n` +
      `🎮 \`!pirate\` — Pirate Treasure Hunt bilow\n` +
      `🔪 \`!dilaay\` — Mafia ciyaar bilow\n` +
      `❓ \`!help\` — Dhammaan amarrada\n\n` +
      `Ku mahadsantahay inaad naga mid noqotay! 🏴\u200d☠️`
    )
    .setThumbnail(member.user.displayAvatarURL({ dynamic: true }))
    .setColor(0x22c55e)
    .addFields(
      { name: '👤 Member', value: `${member.user.tag}`, inline: true },
      { name: '🔢 Tirada', value: `Member #${member.guild.memberCount}`, inline: true },
      { name: '📅 Account', value: `<t:${Math.floor(member.user.createdTimestamp / 1000)}:R>`, inline: true },
    )
    .setFooter({ text: 'Ciyaal Xamar Bot · Soo Dhawoow!' })
    .setTimestamp();
  await channel.send({ embeds: [embed] }).catch(() => null);
});

client.on(Events.GuildMemberRemove, async (member) => {
  pushLog('leave', `${member.user.username} wuxuu ka baxay`, member.guild.name, member.user.id, member.user.username);
});

client.on(Events.GuildCreate, async (guild) => {
  pushLog('guild_join', `Bot wuxuu ku biiray server cusub: ${guild.name} (${guild.memberCount} member)`, guild.name, '', '');
});
